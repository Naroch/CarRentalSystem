...
class EmployeeDamagesControllerTest {
    ...
    void updateDescriptionDamage() throws SQLException {
        ...
        int cost = 450;
        String sql =  "INSERT INTO damage (id_car,description, cost) VALUES(?,?,?) " +
                "ON DUPLICATE KEY UPDATE id_car = '"+id_car+"', description = '"+description+"', cost = '"+cost+"'";
    ...
    void showExistingDamage() throws SQLException {
        ...
        ResultSet resultSet = connection.createStatement().executeQuery("SELECT d.id_damage, d.description, d.cost, p.photo " +
                "FROM damage d " +
                "LEFT JOIN photo_damage pd on d.id_damage = pd.id_damage " +
                "LEFT JOIN photos p on p.id_photo = pd.id_photo " +
                "INNER JOIN cars c on d.id_car = c.id_car " +
        ...
        while (resultSet.next()) {
            ...
            damage.add(resultSet.getString("cost"));
    ...
    void showExistingDamages() throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            damage1.add(resultSet.getString("cost"));
        }
        if (!damage1.isEmpty()) {
