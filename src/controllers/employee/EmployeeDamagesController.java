 ...
 */
public class EmployeeDamagesController implements Initializable {
    ...
    public int car_damage_id;
    public int id_damage;

    /**
     * Inicjalizacja komponentów z pliku EmployeeDamages.fxml
     */
    @FXML
     ...
     * @throws FileNotFoundException wykorzystuję się przy blędnym podaniu pliku
     */
    @FXML
     ...
     */
    @FXML
    void DeleteImageButton(ActionEvent event) {
        try {
            if (id_damage != 0) {
                PreparedStatement delete_statement = DataBase.connect().prepareStatement("DELETE p.*, pd.* FROM photo_damage pd " +
                        "LEFT JOIN photos p on pd.id_photo = p.id_photo WHERE pd.id_damage = '" + id_damage + "'");
     ...
     */
    @FXML
    void DescriptionButton(ActionEvent event) throws SQLException {
        EmployeeShowCarController controller = new EmployeeShowCarController();
        car_damage_id = EmployeeShowCarController.id_car_damage;
        try {
    ...
    /**
     * Dana metoda jest stosowania dla powrotu do poprzedniej sceny (EmployeeShowCar)
     * @param event wykorzysuje się dla obsługi zdarzeń
     */
    @FXML
    ...
    public void showPhotoDamage() throws SQLException, IOException {
        ...
        PreparedStatement statement = DataBase.connect().prepareStatement("SELECT d.id_damage, p.photo FROM damage d " +
                ...
                "WHERE d.id_car = " + EmployeeShowCarController.id_car_damage);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        EmployeeShowCarController controller = new EmployeeShowCarController();
        car_damage_id = EmployeeShowCarController.id_car_damage;
        System.out.println("Car_id: " + car_damage_id);
        ArrayList<Image> images = new ArrayList<Image>();
        Connection connection = DataBase.connect();
        int count_image = 1;
        try {
